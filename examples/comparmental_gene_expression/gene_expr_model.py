...
class GeneExpressionModel:
    ...
    def __init__(self, num_gene_states=4):
        print(f"Comparmental gene expression model with {num_gene_states} gene states.\n")
        ...
        self.stoich_mat = np.zeros(( (num_gene_states - 1)*3 + 2, num_gene_states + 2))
        ...
        self.stoich_mat[(num_gene_states - 1)*2:(num_gene_states - 1)*2 + (num_gene_states - 1), num_gene_states] = 1
        ...
        self.prior_mean[2*(num_gene_states - 1):3*(num_gene_states - 1)] = 1.0
        ...
        # self.theta_ub = np.zeros((3*(num_gene_states - 1) + 2, ))
        # self.theta_lb = np.zeros((3*(num_gene_states - 1) + 2, ))
        #
        # self.theta_ub[0:2*(num_gene_states - 1)] = 1.0E-1
        # self.theta_ub[2*(num_gene_states - 1):3*(num_gene_states - 1)] = 1.0E1
        # self.theta_ub[-2] = 1.0E-1
        # self.theta_ub[-1] = 1.0E-2
        #
        # self.theta_lb[0:2*(num_gene_states - 1)] = 1.0E-4
        # self.theta_lb[2*(num_gene_states - 1):3*(num_gene_states - 1)] = 1.0E-4
        # self.theta_lb[-2] = 1.0E-4
        # self.theta_lb[-1] = 1.0E-4
    ...
    def prop_factory(self, surrogate_id):
        ...
        def propensity(reaction, x, out):
            ...
            if self.num_gene_states - 1 <= reaction < 2*(self.num_gene_states - 1):
                out[:] = x[:, reaction - self.num_gene_states + 2]*active_states
            # mRNA production
            if 2*(self.num_gene_states - 1) <= reaction < 2*(self.num_gene_states - 1) + (self.num_gene_states - 1):
                out[:] = x[:, reaction - 2*(self.num_gene_states - 1) + 1]*active_states
    ...
    def loglike(self, log10_thetas, dataz, surrogate_id):
        ...
        for jnc in range(0, nsamp):
            ...
            for i in range(0, len(t_meas)):
                ll = ll + data[i].LogLikelihood(solutions[i], np.array([self.num_gene_states, self.num_gene_states + 1]))
            loglike[jnc, 0] = ll
        return loglike
...
def simulate_data(t_meas, n_cells):
    ...
    ssa.SetModel(true_model.stoich_mat, true_model.t_fun_factory(theta_true), true_model.prop_factory(modelid))
    print(true_model.stoich_mat)
    observations = []
    for t in t_meas:
        observations.append(ssa.Solve(t, true_model.x0, n_cells, send_to_root=True))
