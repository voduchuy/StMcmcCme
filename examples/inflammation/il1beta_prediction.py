...
if __name__ == "__main__":
    ...
    if rank == 0:
        with np.load(f"il1b_mcmc_{result_file_appendix[method]}.npz") as file:
            Xglobal = file["theta"]
            Xglobal = Xglobal[:, :, -1]
    ...
    if rank == 0:
        ...
        displacements = None
    comm.Scatterv(
        [Xglobal, npar * samples_per_proc, displacements, mpi.DOUBLE], X, root=0
    )
    ...
    comm.Reduce(
        sendbuf=[prna_mean, len(model.t_meas) * veclenmax, mpi.DOUBLE],
        recvbuf=[prna_mean0, len(model.t_meas) * veclenmax, mpi.DOUBLE],
        op=mpi.SUM,
        root=0,
    )
    comm.Reduce(
        sendbuf=[prna_std, len(model.t_meas) * veclenmax, mpi.DOUBLE],
        recvbuf=[prna_std0, len(model.t_meas) * veclenmax, mpi.DOUBLE],
        op=mpi.SUM,
        root=0,
    )
    ...
    if rank == 0:
        ...
        np.savez(
            f"il1b_prediction_{result_file_appendix[method]}.npz",
            mean=prna_mean0,
            std=prna_std0,
        )
