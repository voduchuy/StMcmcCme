...
#SBATCH --nodes=18                # Number of nodes - all cores per node are allocated to the job
...
#SBATCH --job-name=il1b             # Name of job
...
mpiexec --bind-to core --npernode $cores --n $(($cores*$nodes)) python ~/st-mcmc-cme/examples/inflammation/il1beta_stmcmc.py
mpiexec --bind-to core --npernode $cores --n $(($cores*$nodes)) python ~/st-mcmc-cme/examples/inflammation/il1beta_predict.py
