...
class StMcmc_bridge:
    ...
    def Run(self, logprior_fun, loglike_fun, data, init_thetas, outfile=None, nsamp=128, nmodel=1, max_num_steps=1000, maxcount=100):
        ...
        for istep in range(0, self.max_num_steps_):
            oldmodel = modelid
            #before we start lets have everyone compute the next model so we can look at whether we need to change
            if modelid != (nmodel-1):
                sendtheta = None
                counts = None
                dspls = None
                ...
                # compute on the loglike on each core using the next modelid
                sendloglike = self.loglike_(recvtheta, data, modelid+1)
            ...
            if self.rank_ == 0:
                ...
                if modelid != (nmodel-1):
                    #Determine if we need to change models based upon the ESS wrt next model
                    #compute the weights for the current beta
                    maxl = max(beta[istep]*(fullloglike-loglike[:, istep]))
                    wi = np.divide(np.exp(beta[istep] * (fullloglike - loglike[:, istep])-maxl), sum(np.exp(beta[istep] * (fullloglike - loglike[:, istep])-maxl)))
                    fi = np.sqrt(1.0 / nsamp * sum(np.power(wi - np.mean(wi), 2))) / np.mean(wi) - self.targcov_
            ...
            if modelid != oldmodel:
                ...
                if self.rank_ == 0:
                    newloglike = fullloglike
                    maxl = max(beta[istep+1]*(newloglike-loglike[:,istep]))
                    wi = np.divide(np.exp(beta[istep+1] * (newloglike - loglike[:, istep])-maxl), sum(np.exp(beta[istep+1] * (newloglike - loglike[:, istep])-maxl)))
