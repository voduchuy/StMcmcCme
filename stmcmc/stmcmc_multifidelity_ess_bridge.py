...
class StMcmcMultiFidelityEssBridge:
    ...
    def Run(
        ...
        """
        Sample from the posterior distribution using the ESS-based Multifidelity STMCMC sampler.
        ...
        for istep in range(0, max_num_steps):
            ...
            if self.rank_ == 0:
                ...
                if modelid != max_level:
                    ...
                    # compute the weights for the current betas
                    maxl = np.max(betas[istep] * (fullloglike[:,0] - loglike[:, istep]))
                    wi = np.divide(
                        np.exp(
                            betas[istep] * (fullloglike[:,0] - loglike[:, istep])
                            - maxl
                        ),
                        sum(
                            np.exp(
